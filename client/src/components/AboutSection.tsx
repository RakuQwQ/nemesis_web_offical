...
/** Fallback shown if the Discord API is unreachable. */
const MEMBER_COUNT_FALLBACK = 600;

const stats: { icon: typeof Users; value: number | string; suffix: string; label: string }[] = [
...
export default function AboutSection() {
  ...
  const { data: discord } = useDiscordStats();
  const memberValue = discord?.memberCount ?? MEMBER_COUNT_FALLBACK;
  // Use a "+" suffix only when we're falling back to the rounded estimate.
  const memberSuffix = discord?.memberCount ? '' : '+';

  const liveStats: typeof stats = [
    { icon: Users, value: memberValue, suffix: memberSuffix, label: '公會成員' },
    ...stats,
  ];
